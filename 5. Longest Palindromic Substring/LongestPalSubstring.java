...
//Intuition :
//To avoid the unnecessary validation of palindromes, we can use Manacher's algorithm. 
...
//We use previously calculated values to determine if we can expand around the center or not. If we can expand, we expand and update the center and right boundary. 
//Otherwise, we move to the next character and repeat the process. We also maintain a variable max_len to keep track of the maximum length of the palindrome. 
//We also maintain a variable max_str to keep track of the maximum substring.
